...
//   const [recommendations, setRecommendations] = useState([]);
  ...
  const handleUsernameAChange = (event) => {
    setUsernameA(event.target.value);
  };
  const handleUsernameBChange = (event) => {
...
//   };
  ...
  return (
    <div className="flex flex-row items-center justify-center min-h-screen bg-black text-white p-4">
      <main className="container mx-flex p-4">
        ...
        <form onSubmit={handleSubmit} className="mb-8">
          ...
          <input
            ...
            name="username"
            value={usernameA}
            onChange={handleUsernameAChange}
            placeholder="Enter your Spotify username"
            className="w-half p-2 rounded bg-white text-black mb-4"
            required
          />
          <label htmlFor="username" className="block mb-2 text-sm font-bold">
            Their Spotify Username:
          </label>
          <input
            type="text"
            id="username"
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? "Loading..." : "Find Similarity"}
          </button>
        </form>
